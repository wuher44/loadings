...
public interface DriverRepository extends JpaRepository<Driver, Long> {
    ...
    List<Driver> findAllByOrderByLastnNameAsc();

    @Query("select d from  Driver d where (d.firstName like %:phrase% or d.lastName like %:phrase%) and d.status = :status ")
    List<Driver> searchByPhraseAndStatus(@Param("phrase") String phrase, @Param("status") Status status);
